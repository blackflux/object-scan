...
const markExcluded = (input: any) => defineProperty(input, IS_EXCLUDED, true);
const isExcluded = (input: any) => input[IS_EXCLUDED] === true;

const throwError = (msg: string, input: string, context = {}) => {
  throw new Error(Object.entries(context)
    .reduce((p, [k, v]) => `${p}, ${k} ${v}`, `${msg}: ${input}`));
};

const getSimple = (arrOrSet: any[] | Set<any>) => {
...
class CString extends String {
  ...
  constructor(value: string, excluded: boolean) {
    super(value);
...
const Result = (input: any) => {
  ...
  return {
    setInArray: (flag: boolean, idx: any) => {
      if (inArray === flag) {
        throwError(inArray ? 'Bad Array Start' : 'Bad Array Terminator', input, { char: idx });
      }
      inArray = flag;
    },
    finishElement: (idx: number, { err, fins, finReq = false }: any) => {
