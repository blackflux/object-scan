...
const parser_1 = __importDefault(require("../../src/util/parser"));
const asString = (() => {
    ...
    return (input) => asStringRec(parser_1.default(input));
})();
const checkError = (input, msg) => {
    chai_1.expect(() => parser_1.default(input)).to.throw(msg);
};
node_tdd_1.describe('Testing Parser', () => {
    node_tdd_1.describe('Complex Use Cases', () => {
        it('Testing Path Groups', () => {
            chai_1.expect(asString('{a,b.c}')).to.equal('{"a",["b","c"]}');
        });
        it('Testing Nested Groups', () => {
            chai_1.expect(asString('{a,{b,c}}')).to.deep.equal('{"a","b","c"}');
            chai_1.expect(asString('{a,{b,{c}}}')).to.deep.equal('{"a","b","c"}');
        });
        it('Testing Array Group Content', () => {
            chai_1.expect(asString('[{1,{0,1}}]')).to.deep.equal('{"[1]","[0]","[1]"}');
            chai_1.expect(asString('[{{0,1},1}]')).to.deep.equal('{"[0]","[1]","[1]"}');
        });
    });
    node_tdd_1.describe('Testing Simple Use Cases', () => {
        it('Testing Empty', () => {
            chai_1.expect(asString('')).to.deep.equal('""');
        });
        it('Testing Simple', () => {
            chai_1.expect(asString('a')).to.deep.equal('"a"');
        });
        it('Testing Path', () => {
            chai_1.expect(asString('a.b')).to.deep.equal('["a","b"]');
        });
        it('Testing Array', () => {
            chai_1.expect(asString('a[0]')).to.deep.equal('["a","[0]"]');
        });
        it('Testing Or', () => {
            chai_1.expect(asString('{a,b}')).to.deep.equal('{"a","b"}');
        });
        it('Testing Comma Outside Group', () => {
            chai_1.expect(asString('a,b')).to.deep.equal('{"a","b"}');
            chai_1.expect(asString('a.b,c.d')).to.deep.equal('{["a","b"],["c","d"]}');
        });
        it('Testing Or In Array', () => {
            chai_1.expect(asString('[{0,1}]')).to.deep.equal('{"[0]","[1]"}');
        });
        it('Testing Array In Or', () => {
            chai_1.expect(asString('{[0],[1]}')).to.deep.equal('{"[0]","[1]"}');
        });
        it('Testing Array in Path', () => {
            chai_1.expect(asString('a.*.c[0]')).to.deep.equal('["a","*","c","[0]"]');
        });
        it('Testing Array After Or', () => {
            chai_1.expect(asString('{a,b}[0]')).to.deep.equal('[{"a","b"},"[0]"]');
        });
    });
    ...
    node_tdd_1.describe('Invalid Dot Selector', () => {
    ...
    node_tdd_1.describe('Array Selector', () => {
    ...
    node_tdd_1.describe('Simple Group Selector', () => {
    ...
    node_tdd_1.describe('Testing Exclusion', () => {
        it('Testing Basic Exclusion', () => {
            ...
            chai_1.expect(asString('{!a,b.c}')).to.equal('{!"a",["b","c"]}');
            chai_1.expect(asString('!{a,b.c}')).to.equal('{!"a",[!"b","c"]}');
            chai_1.expect(asString('!a.!b')).to.equal('[!"a",!"b"]');
            chai_1.expect(asString('[!0][!1]')).to.equal('[!"[0]",!"[1]"]');
            chai_1.expect(asString('!{{a,b},{c,d}}')).to.equal('{!"a",!"b",!"c",!"d"}');
        });
        it('Testing Array Exclusion', () => {
            chai_1.expect(asString('{[0],[!1][2]}')).to.equal('{"[0]",[!"[1]","[2]"]}');
            chai_1.expect(asString('{[0],[1][!2]}')).to.equal('{"[0]",["[1]",!"[2]"]}');
            chai_1.expect(asString('{[!0],[1][2]}')).to.equal('{!"[0]",["[1]","[2]"]}');
            chai_1.expect(asString('{[0],![1][2]}')).to.equal('{"[0]",[!"[1]","[2]"]}');
            chai_1.expect(asString('![1][{2,3}][4]')).to.equal('[!"[1]",{"[2]","[3]"},"[4]"]');
            chai_1.expect(asString('!{[0][1],[1][2]}')).to.equal('{[!"[0]","[1]"],[!"[1]","[2]"]}');
        });
        it('Testing Exclusion in Path Group', () => {
            chai_1.expect(asString('**.{*,!location}.lat'))
        ...
        it('Nested Redundant Exclusions', () => {
            chai_1.expect(asString('!{[1][2],*,{a,b},{a},{{a}},{a.!b}}'))
                .to.equal('{[!"[1]","[2]"],!"*",!"a",!"b",!"a",!"a",[!"a",!"b"]}');
        });
        node_tdd_1.describe('Testing Exclusion Errors', () => {
