...
describe('Testing Find', () => {
  ...
  it('Testing Arbitrary Location Nested Array', () => {
    const find: any = objectScan(['**.nestedArray[*][1]']);
    expect(find({
      key: {
  ...
  describe('Testing Exclusion', () => {
    const execute = (input: any, needles: any, result: any) => expect(objectScan(needles)(input)).to.deep.equal(result);
  ...
  describe('Testing empty needle behaviour', () => {
    const needles = [''];
    const arrayInput: any = [{ id: 1 }, { id: 2 }];
    const objectInput: any = { id: {} };
    const filterFn = (ps: any = []) => (key: any, value: any, { isMatch, matchedBy, parents }: any) => {
    ...
    it('Testing array objects with useArraySelector === false (nested)', () => {
      const find: any = objectScan(needles, {
    ...
    it('Testing empty needle only matchedBy on top level', () => {
      const find = objectScan(['', '**'], {
        useArraySelector: false,
        filterFn: (key: any, value: any, { matchedBy }: any) => matchedBy.includes('')
      });
      expect(find(arrayInput)).to.deep.equal(['[1]', '[0]']);
    });
  ...
  it('Testing null value', () => {
    const find: any = objectScan(['**'], { filterFn: (key: any, value: null) => value === null });
    expect(find({ key: null })).to.deep.equal(['key']);
  });

  it('Testing undefined value', () => {
    const find: any = objectScan(['**'], { filterFn: (key: any, value: undefined) => value === undefined });
  ...
  describe('Testing sorting', () => {
    it('Testing array matches reversed', () => {
      const find: any = objectScan(['**'], { joined: false });
  ...
  describe('Testing Fn parents', () => {
    ...
    describe('Testing Array Target', () => {
      const input: any = { one: { child: ['a', 'b', 'c'] } };
      const pattern = ['**.child'];
  ...
  describe('Testing useArraySelector + breakFn', () => {
    const input: any = { child: [{ id: 1 }] };
    const pattern = ['child.id', 'child[0].id'];

    const execTest = (useArraySelector: any, breakFn: any) => {
      const result: any = [];
      objectScan(pattern, {
        breakFn: (k: any) => {
          result.push(k);
    ...
    it('Testing useArraySelector = false, breakFn (BREAKING)', () => {
      expect(execTest(false, (k: string) => k === 'child')).to.deep.equal(['', 'child']);
    });
    ...
    it('Testing useArraySelector = true, breakFn (BREAKING)', () => {
      expect(execTest(true, (k: string) => k === 'child')).to.deep.equal(['', 'child']);
    });
    ...
    it('Testing useArraySelector = false invokes breakFn but not filterFn', () => {
      ...
      objectScan(['**'], {
        useArraySelector: false,
        filterFn: (k: any) => result.push(['filterFn', k]),
        breakFn: (k: any) => result.push(['breakFn', k])
      })({
        tag: [[{ id: 1 }]]
      });
      expect(result).to.deep.equal([
  ...
  describe('Testing Fn traversedBy', () => {
    ...
    it('Testing traversedBy on filterFn', () => {
      const result: any = [];
      objectScan(pattern, {
        filterFn: (k: any, v: any, { traversedBy }: any) => result.push(`${traversedBy} => ${k}`)
      })(input);
      expect(result).to.deep.equal([
    ...
    it('Testing traversedBy on breakFn', () => {
      const result: any = [];
      objectScan(pattern, {
        breakFn: (k: any, v: any, { isMatch, traversedBy }: any) => result.push(`${traversedBy} => ${k} (${isMatch})`)
      })(input);
      expect(result).to.deep.equal([
        '[*].*.child,[*].parent =>  (false)',
  ...
  describe('Testing Fn needle', () => {
    ...
    it('Testing matchedBy on filterFn', () => {
      const result: any = [];
      objectScan(pattern, {
        filterFn: (k: any, v: any, { matchedBy }: any) => result.push(`${matchedBy} => ${k}`)
      })(input);
      expect(result).to.deep.equal([
        '[*].*.child => [0].parent.child',
        '[*].parent => [0].parent'
    ...
    it('Testing matchedBy on breakFn', () => {
      const result: any = [];
      objectScan(pattern, {
        breakFn: (k: any, v: any, { matchedBy }: any) => result.push(`${matchedBy} => ${k}`)
      })(input);
      expect(result).to.deep.equal([
        ' => ',
        ' => [0]',
  ...
  it('Testing Filter Function', () => {
    const find = objectScan(['**'], {
      filterFn: (key: any, value: string) => typeof value === 'string' && value === 'a'
    });
    expect(find(haystack)).to.deep.equal([
      'array2.nested[0]',
      'array1[0]',
  ...
  it('Testing Readme Example', () => {
    ...
    expect(objectScan(['**'], { filterFn: (key: any, value: any) => typeof value === 'string' })(input)).to
      .deep.equal(['k', 'a.h[1]', 'a.h[0]', 'a.e.f', 'a.b.c']);
    expect(objectScan(['**'], { breakFn: (key: string) => key === 'a.b' })(input)).to
